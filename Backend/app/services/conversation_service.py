...
class ConversationService:
    ...
    async def find_conversation(self, conversation_id: str) -> Conversation | None:
        return await self._conversation_repository.find_by_id(
            conversation_id=conversation_id
        )

    async def get_conversation(self, conversation_id: str) -> Conversation:
        conversation: Conversation | None = await self.find_conversation(
            conversation_id=conversation_id
        )
    ...
    async def delete_conversation(self, conversation_id: str) -> bool:
        return await self._conversation_repository.delete(
            conversation_id=conversation_id
        )